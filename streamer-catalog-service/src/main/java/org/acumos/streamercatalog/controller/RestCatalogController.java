...
@Controller
@RequestMapping(value = "v1/streamers", produces = MediaType.APPLICATION_JSON)
public class RestCatalogController {
	...
	private static final String CODECLOUD_AUTHORIZATION ="CodeCloudAuthorization";
	...
	@ResponseBody
	public Response saveCatalog(@RequestHeader(AUTHORIZATION) String authorization,
			@RequestHeader(CODECLOUD_AUTHORIZATION) String codeCloudAuthorization,
			@RequestBody CatalogObject objCatalog) {
		
		logger.debug("in save catalog of RestCatalogController: ");
		return service.saveCatalog(authorization, codeCloudAuthorization, objCatalog);
	}
	...
	@ApiResponses(value = { @ApiResponse(code = 201, message = UPDATED),
			...
			@ApiResponse(code = 500, message = UNEXPECTED_RUNTIME_ERROR) })
	@ResponseBody
	public Response updateCatalog(@RequestHeader(AUTHORIZATION) String authorization,
			@RequestHeader(CODECLOUD_AUTHORIZATION) String codeCloudAuthorization,
			@PathVariable(STREAMER_KEY) String catalogKey, @RequestBody CatalogObject objCatalog) {
		
		logger.debug("in update catalog of RestCatalogController: ");
		return service.updateCatalog(authorization, codeCloudAuthorization, catalogKey, objCatalog);
	}
	...
	@ApiResponses(value = { @ApiResponse(code = 200, message = OK), @ApiResponse(code = 400, message = BAD_REQUEST),
			...
			@ApiResponse(code = 500, message = UNEXPECTED_RUNTIME_ERROR) })
	@ResponseBody
	public Response getCatalog(@RequestHeader(AUTHORIZATION) String authorization,
			@PathVariable(STREAMER_KEY) String catalogKey, @RequestParam(MODE) String mode) {
		
		logger.debug("in getcatalog of RestCatalogController: ");
	...
	@ApiResponses(value = { @ApiResponse(code = 200, message = OK), @ApiResponse(code = 400, message = BAD_REQUEST),
			...
			@ApiResponse(code = 500, message = UNEXPECTED_RUNTIME_ERROR) })
	@ResponseBody
	public Response getCatalogs(@RequestHeader(AUTHORIZATION) String authorization,
			@RequestParam(CATEGORY) String category, @RequestParam(TEXT_SEARCH) String textSearch) {
		
		logger.debug("in getcatalogs of RestCatalogController: ");
	...
	@ApiResponses(value = { @ApiResponse(code = 204, message = NO_CONTENT),
			...
			@ApiResponse(code = 500, message = UNEXPECTED_RUNTIME_ERROR) })
	@ResponseBody
	public Response deleteCatalog(@RequestHeader(AUTHORIZATION) String authorization,
			@PathVariable(STREAMER_KEY) String catalogKey) {
		
		logger.debug("in deletecatalog of RestCatalogController: ");
