...
public class DataStreamerCatalogUtil {
	...
	public String getComponentPropertyValue(String key) throws IOException {
		...
		InputStream input;
		if (new File(System.getProperty(USER_DIR) + System.getProperty(FILE_SEPARATOR) + CONFIG_PROPERTIES).exists()) {
	...
	public String readAttachedFileContents(Attachment attachment) {
		...
		try {
			...
			String tmpFileName = System.getProperty(USER_DIR) + System.getProperty(FILE_SEPARATOR) + attachmentfilename
					+ TMP;
	...
	public JSONObject getModelDetails(String authorization, String modelKey) throws IOException, DataStreamerException {
		...
		logger.info(
				"HelperTool::getModelDetails()::prepping request to model manager to get predictorId using modelKey: "
						+ modelKey);
		...
		logger.info("HelperTool::getModelDetails()::response from model manager service is "
				+ response.getStatusLine().getStatusCode() + " for model key: " + modelKey);

		StringBuilder modelDetails = new StringBuilder();

		if (response.getStatusLine().getStatusCode() == 200) {
			logger.info("HelerTool::getModelDetails()::Received OK response from model manager URL for predictor: "
					+ modelKey);
			...
			logger.info(
					"HelerTool::getModelDetails()::Received Bad response 401 from model manager. Throwing exception...");
			throw new DataStreamerException(
					"OOPS. The model manager didn't authorise for details of the model with key as :" + modelKey
							+ ". Please ask admin to provide access to get credentials.");
		} else {
			logger.info(
					"HelerTool::getModelDetails()::Received Bad response" + response.getStatusLine().getStatusCode());
			logger.info(
					"HelerTool::getModelDetails()::Received Bad response  (other than 401) from model manager. Throwing exception...");
		...
		try {
			...
			logger.info(
					"HelerTool::getModelDetails()::ignoring the exception during release connection of http request.");
		}
		return new JSONObject(modelDetails.toString());
	}

	public JSONObject getPredictorDetails(String authorization, String predictorId)
			throws IOException, DataStreamerException {

		logger.info(
				"HelperTool::getPredictorDetails()::prepping request to predictor manager to get info on predictor: "
						+ predictorId);
		HttpClient client = HttpClients.createDefault();
		...
		HttpResponse response = client.execute(request);
		logger.info("HelperTool::getPredictorDetails()::response from predictor manager service is "
				+ response.getStatusLine().getStatusCode() + " for predictor key: " + predictorId);

		StringBuilder predictorDetails = new StringBuilder();

		if (response.getStatusLine().getStatusCode() == 200) {
			logger.info(
					"HelerTool::getPredictorDetails()::Received OK response from predictor manager URL for predictor: "
							+ predictorId);
			BufferedReader rd = new BufferedReader(new InputStreamReader(response.getEntity().getContent()));
			...
			logger.info(
					"HelerTool::getPredictorDetails()::Received Bad response 401 from predictor manager. Throwing exception...");
			throw new DataStreamerException(
					"OOPS. The predcitor manager didn't authorise for details of the preictor with key as :"
							+ predictorId + ". Please ask admin to provide access to get credentials.");
		} else {
			logger.info("HelerTool::getPredictorDetails()::Received Bad response"
					+ response.getStatusLine().getStatusCode());
			logger.info(
					"HelerTool::getPredictorDetails()::Received Bad response  (other than 401) from predictor manager. Throwing exception...");
		...
		try {
			...
			logger.info(
					"HelerTool::getPredictorDetails()::ignoring the exception during release connection of http request.");
		}
		return new JSONObject(predictorDetails.toString());
	}

	// TO DO:if this method required or not
	public JSONObject getPredictorUrl(String authorization, String predictorId)
			throws IOException, DataStreamerException {
		// String pmsUrl = HelperTool.getEnv("pms_url",
		// HelperTool.getComponentPropertyValue("pms_url"));
	...
	public void validateRequest(String user, CatalogObject objCatalog) throws DataStreamerException, Exception {
		...
		if ((objCatalog.getCategory() != null) || objCatalog.getCategory().isEmpty()) {
			...
			if (objCatalog.getCategory().equalsIgnoreCase(MSG_ROUTER)
					&& (!(objCatalog.getSubscriberUrl() != null) || objCatalog.getSubscriberUrl().isEmpty())) {
			...
			if (objCatalog.getCategory().equalsIgnoreCase(MSG_ROUTER) && (objCatalog.getPollingInterval() % 5 != 0)) {
				throw new DataStreamerException(
						INVALID_POLLING_INTERAVL_PLEASE_USE_5_OR_MULTIPLES_OF_5_MINUTES_AS_VALUE, 400);
			}
		}
	}
