...
public class PublisherSubscriberController {
	...
	@ApiResponses(value = { @ApiResponse(code = 201, message = CREATED),
			...
			@ApiResponse(code = 403, message = FORBIDDEN), @ApiResponse(code = 404, message = SERVICE_NOT_AVAILABLE),
			@ApiResponse(code = 500, message = UNEXPECTED_RUNTIME_ERROR) })
	@ResponseBody
	public ResponseEntity sendMessageToKafkaTopic(@RequestHeader(AUTHORIZATION) String authorization,
			@RequestHeader(TOPIC) String topic, @RequestBody String msgs) {
	...
	@ApiResponses(value = { @ApiResponse(code = 200, message = OK), @ApiResponse(code = 400, message = BAD_REQUEST),
			...
			@ApiResponse(code = 500, message = UNEXPECTED_RUNTIME_ERROR) })
	@ResponseBody
	public ResponseEntity recieveMessageFromKafkaTopic(@RequestHeader(AUTHORIZATION) String authorization,
			@RequestHeader(TOPIC) String topic, @PathVariable(GROUP_NAME) String groupName,
