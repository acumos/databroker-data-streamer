...
public class PublisherImpl {
	...
	private void loadredir() throws IOException {
		
		BufferedReader br = null;
		try {
			br = new BufferedReader(new FileReader(redirfile));
	...
	private void saveredir() throws IOException {
		
		OutputStream os = null;
		try {
			os = new FileOutputStream(redirfile);
	...
	private synchronized void clearredir(String saveurl) throws IOException {
		if (url == saveurl) {
			url = origurl;
			redircnt = 0;
			saveredir();
	...
	private synchronized String adjustredir(String saveurl, String location, String fileid) throws IOException {
		if (url == saveurl) {
			if (redircnt >= 2) {
				url = origurl;
				redircnt = 0;
